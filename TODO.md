...
- [ ] As any user in the system, I should see a kind of "Status" of each workspace as colours (Green = Online, Red = Unreachable/Token expired, etc) in the workspace view dashboard. This status should update whenever anyone updates the workspace details. Also, there should be a background job running to ping the "Test connection" endpoint every hour for each workspace. Also, if some filter/search is executed for any workspace anywhere and it is found at that instance that the workspace is unreachable, it should immediately reflect in the dashboard.
