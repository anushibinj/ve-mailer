...
- [ ] Add a new custom filter field called "Triage SLA". When this field is added, the filter should also fetch the "creation_time" field of the tickets. You can refer to the existing logic for the "AI Summary" custom field in com.anushibinj.veemailer.service.FilterService. Now, the output of Triage SLA field is a traffic light (in the frontend and the e-mail). If the ticket was created 7+ days ago, it should be red 🔴. 3-4 days ago should be yellow 🟡. 0-2 days ago should be green 🟢. Show the number of days also after the traffic light. Example, "🟡 3 days old". Also, when "Triage SLA" field is selected, the result should be sorted in descending order for the number of days since ticket creation. So, tickets that are untriaged for a longer period of time will bubble to the top. Make this day-based-traffic-light logic simpler to edit because the higher org might ask me to change the days range later for each colour.
