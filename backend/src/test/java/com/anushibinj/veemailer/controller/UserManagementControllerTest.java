...
class UserManagementControllerTest {
    ...
    void testDeleteUser_returnsSuccessWrapper() throws Exception {
        UUID userId = UUID.randomUUID();
        when(authService.deleteUserBySuperAdmin(org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.eq(userId)))
                .thenReturn("User deleted successfully.");

        mockMvc.perform(delete("/api/admin/users/{userId}", userId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
