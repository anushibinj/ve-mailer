...
Use the ticket's waiting-days value from `phase_age`, which is provided directly as {phaseAge} days. Do not compute waiting time from `creation_time`.
...
2. Today's date and time is {todaydatetime}. The ticket has been in its current phase for {phaseAge} days (`phase_age`) — report the waiting duration using this value (for example, "waiting for {phaseAge} days"). Do not compute this from `creation_time`, and do not reiterate today's date in the final output.
